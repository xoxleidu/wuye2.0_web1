...
<script>
import { getPlace, updatePlace } from "@/api/index.js";
export default {
  data() {
    return {
      loading: false,
  ...
  created() {
    ...
    getPlace({ placeId: this.$props.id }).then(res => {
      if (res.data.code == 0) {
        this.$set(this.$data, "postData", res.data.data);
      }
    });
  },
  methods: {
